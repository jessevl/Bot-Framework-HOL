...
var BINGNEWSKEY = 'YOURKEYHERE';
var VISIONKEY = 'YOURKEYHERE';
...
var luisRecognizer = new builder.LuisRecognizer('YOURLUISURL');
...
bot.dialog('/analyseImage', [
    function (session){
        // Ask the user which category they would like
        // Choices are separated by |
        builder.Prompts.text(session, "Send me a link to an image to analyze...");
    }, function (session, results, next){
        // The user chose a category
        if (results.response && results.response.entity !== '(quit)') {
           //Show user that we're processing their request by sending the typing indicator
            session.sendTyping();
            // The url we'll be calling to get the result
...
// This function processes the results from the API call to category news and sends it as cards
function sendAnalysis(session, body){
    session.send("The description for this image:");
    //Show user that we're processing by sending the typing indicator
    session.sendTyping();
    // The value property in body contains an array of all the returned articles
